...
class BaseRssIngestor:
    ...
    def __init__(self, feed_url, home_url, source_name):
        ...
        # AWS S3 Client (IAM role must be attached on EC2)
        try:
            self.s3 = boto3.client("s3")
        except NoCredentialsError:
            raise RuntimeError("❌ No AWS credentials found. Attach IAM role to EC2.")
        ...
        try:
            if link and link.endswith(".pdf"):
                ...
                if res.status_code == 200:
                    ...
                    # ✅ Debug print
                    print(f"✅ PDF added to S3: s3://{self.bucket}/{key}")
                    logging.info(f"PDF added to S3: s3://{self.bucket}/{key}")
                    
                    return f"s3://{self.bucket}/{key}", "Completed"
                else:
                    print(f"⚠️ PDF download failed ({res.status_code}): {link}")
            else:
                ...
                # ✅ Debug print
                print(f"ℹ️ No PDF, saved placeholder TXT: s3://{self.bucket}/{key}")
                logging.info(f"No PDF, saved TXT to S3: s3://{self.bucket}/{key}")
...
class NseRssIngestor(BaseRssIngestor):
    ...
    def fetch_and_process(self):
        ...
        try:
            ...
            soup = BeautifulSoup(res.content, "lxml-xml")  # FIXED parser
            ...
            for item in items:
                ...
                if 1 not in market_ids:  # Only ingest NSE if Market ID 1
                    continue
...
class BseRssIngestor(BaseRssIngestor):
    ...
    def fetch_and_process(self):
        ...
        try:
            ...
            root = BeautifulSoup(xml_text, "lxml-xml")  # FIXED parser
            ...
            for item in items:
                ...
                if 2 not in market_ids or 1 in market_ids:  # Only ingest BSE if Market ID 2 only
                    continue
