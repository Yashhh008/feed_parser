...
# Ensure lxml parser is available for BeautifulSoup
try:
    from lxml import etree  # just to ensure lxml is installed
except ImportError:
...
class BseRssIngestor(BaseRssIngestor):
    ...
    def fetch_and_process(self):
        ...
        try:
            res = self.session.get(self.feed_url, timeout=10)
            res.raise_for_status()
            root = BeautifulSoup(res.content, "xml")
            ...
            for item in items:
                ...
                if pub_date_str:
                    try:
                        pub_date_dt = parsedate_to_datetime(pub_date_str).astimezone(self.ist)
